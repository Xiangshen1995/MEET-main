...
def main(args):
    ...
    if os.path.exists("./weights/CHB/" + args.task) is False:
        os.makedirs("./weights/CHB/" + args.task)
    if os.path.exists("./Results/CHB/") is False:
        os.makedirs("./Results/CHB/")

    tb_writer = SummaryWriter("./Summary/CHB/" + args.task + "/")

    # Load data
    for sbj in range(1,2):
        #raw_data = sio.loadmat("./dataset/SEED/eeg_convert_to_image/S" + str(sbj) + "_32@32.mat")
        # Change to your dataset path
        raw_data = sio.loadmat("./dataset/CHB-MIT/convertImage/chb" + str(sbj) + "_1time-step_32@32.mat")
    ...
    EEG_Images = np.transpose(All_data, (0,2,1,3,4)) # change 2 and 1?
    ...
    for epoch in range(args.epochs):
        ...
        if best_acc < val_acc:
            ...
            torch.save(model.state_dict(), "./weights/CHB/" + args.task + "/best_model.pth")

            best_acc = val_acc
        
        # Save the results
        #result_file = os.path.join("./Results/SEED/", 'Result_%s.mat'%args.task)
        result_file = os.path.join("./Results/CHB/", 'Result_%s.mat'%args.task)
...
if __name__ == '__main__':
    ...
    # The path of pretrained weight, set to null if you don't want to load it,./weights/SEED/SEED_S1/best_model.pth
    parser.add_argument('--weights', type=str, default='./weights/CHB/CHB_S1/best_model.pth',
